...
namespace VCM_FullAssy.Processing
{
    public partial class CRootProcess : ProcessingBase
    {
        ...
        public CTrayProcess LeftTrayProcess { get; set; }
        public CTrayProcess RightTrayProcess { get; set; }
        ...
        public ITrayModel CurrentLoadTray
        {
            get
            {
                return LeftTrayProcess.InWorking ? LeftTrayProcess.CurrentLoadTray : RightTrayProcess.CurrentLoadTray;
            }
        }
        ...
        public ITrayModel CurrentUnloadTray
        {
            get
            {
                return LeftTrayProcess.InWorking ? LeftTrayProcess.CurrentUnloadTray : RightTrayProcess.CurrentUnloadTray;
            }
        }
        #endregion
        ...
        public void InitProcess()
        {
            ...
            LeftTrayProcess = new CTrayProcess(this, "LTray", MessageCode + 2000, IntervalTime = this.IntervalTime + 2)
            {
                FriendlyName = "Left Tray",
            };
            RightTrayProcess = new CTrayProcess(this, "RTray", MessageCode + 2500, IntervalTime = this.IntervalTime + 2)
            {
                FriendlyName = "Right Tray",
            };
            TransferProcess = new CTransferProcess(this, "Trans", MessageCode + 3000, IntervalTime = this.IntervalTime + 2);
            HeadProcess = new CHeadProcess(this, "Head", MessageCode + 3500, IntervalTime = this.IntervalTime + 2);
            UpperVisionProcess = new CUpperVisionProcess(this, "UpperV", MessageCode + 4000, IntervalTime = this.IntervalTime + 2);
            UnderVisionProcess = new CUnderVisionProcess(this, "UnderV", MessageCode + 4500, IntervalTime = this.IntervalTime + 2);

            Childs = new ObservableCollection<IProcessing>();

            this.Childs.Add(LeftTrayProcess);
            this.Childs.Add(RightTrayProcess);
        ...
        public override PRtnCode ProcessToRun()
        {
            ...
            {
                // Only set working tray when PGM first start
                // Keep trays InWorking status when STOP -> START machine
                if (LeftTrayProcess.InWorking == false && RightTrayProcess.InWorking == false)
                {
                    LeftTrayProcess.InWorking = true;
                    RightTrayProcess.InWorking = false;
                }

                LeftTrayProcess.InWorkingChanged = (s, e) =>
                {
                    if (LeftTrayProcess.InWorking == false) RightTrayProcess.InWorking = true;
                };
                RightTrayProcess.InWorkingChanged = (s, e) =>
                {
                    if (RightTrayProcess.InWorking == false) LeftTrayProcess.InWorking = true;
                };
