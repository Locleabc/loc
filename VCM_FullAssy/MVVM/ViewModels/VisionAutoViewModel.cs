        ...
        public RelayCommand GrabCommand
        {
            get
            {
                return new RelayCommand((o) =>
                {
                    ...
                    {
                        case EVisionArea.LOAD:
                            CDef.TopCamera.ExposureTime = CDef.UpperVisionRecipe.LoadVision_ExposureTime;
                        ...
                        case EVisionArea.UNDER:
                            ...
                            break;
                        case EVisionArea.UNLOAD:
                            CDef.TopCamera.ExposureTime = CDef.UpperVisionRecipe.UnloadVision_ExposureTime;
            ...
            CDef.MainViewModel.InitVM.InitCompletedEvent += (s, e) =>
            {
                CDef.TopCamera.GrabFinished += (_s, _e) =>
                {
                    if (CDef.TopCamera.GrabResult.RtnCode == EGrabRtnCode.OK)
                    {
                        if (CDef.TopCamera.IsLive)
                        {
                            ...
                            {
                                // Display Live Image
                                OutputMat_TOP = CDef.TopCamera.GrabImage;
                            }
                            else if (CDef.TopCamera.GrabTarget == "GrabBase")
                            {
                                // Display Live Image
                                OutputMat_BASE = CDef.TopCamera.GrabImage;
                            }
                        }
                        else
                        {
                            if (CDef.TopCamera.GrabTarget == "GrabTop")
                            {
                                InputMat_TOP = CDef.TopCamera.GrabImage;
                            ...
                            {
                                InputMat_BASE = CDef.TopCamera.GrabImage;
                ...
                CDef.BotCamera.GrabFinished += (_s, _e) =>
                {
                    if (CDef.BotCamera.GrabResult.RtnCode == EGrabRtnCode.OK)
                    {
                        ...
                        {
                            // Display Live Image
                            OutputMat_BOT = CDef.BotCamera.GrabImage;
                        }
                        else
                        {
                            InputMat_BOT = CDef.BotCamera.GrabImage;
